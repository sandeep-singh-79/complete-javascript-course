...
function calculate_tip_avg(tips) {
  let tip_sum = 0;
  for (let i = 0; i < tips.length; i++) tip_sum += tips[i];
  return tip_sum / tips.length;
}

john.calculate_tip();
mark.calculate_tip();

john.average = calculate_tip_avg(john.tips);
mark.average = calculate_tip_avg(mark.tips);
if (john.average > mark.average)
  console.log(
    `John's family paid the higher tip amount at ${john.average} on average`
  );
else if (mark.average < mark.average)
  console.log(
    `Marks's family paid the higher tip amount at ${mark.average} on average`
  );
else
  console.log(
